...
- **Next (autonomous increments):** keep raising **fidelity + editing UX** — smoother selection/move/resize, text-annotation editing, stamps/images, opacity/font controls, more polish. Collaboration/Share is **deprioritized** per the user; revisit later (collab notes from the Phase-3 investigation are in git history / the `services/collab` audit).
