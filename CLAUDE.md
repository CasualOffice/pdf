...
- **Next (autonomous increments):** keep raising **fidelity + editing UX** — smoother selection/move/resize, text-annotation inline editing, opacity/font controls, more polish; redaction true-removal (UX-S5) remains open (needs engine/Rust-core). Collaboration/Share is **deprioritized** per the user; revisit later (collab notes from the Phase-3 investigation are in git history / the `services/collab` audit).
