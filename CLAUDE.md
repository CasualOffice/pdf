...
- **Google-Docs-style layout (done).** App bar (logo + title + Share stub + avatar) in `apps/web`; left-aligned rounded toolbar in the SDK; view/edit/suggest as a Docs-style **mode dropdown** (`src/ui/chrome.tsx` `ModeMenu`); mode controlled by the host via `CasualPdfProps.onModeChange`. Brand **logo + favicon** from `apps/web/public/logo.svg` (generated by `tools/brand/gen-icons.mjs`). Competitive landscape gained **iLovePDF** (server-side tool-suite; no live co-editing — our wedge) in `docs/RESEARCH.md`. See [[feedback-ui-design-prefs]].
...
- **Next (autonomous increments — user wants all):** property bar (color/width), then real Share dialog + presence (Phase 3 collab), more visual polish, Format/Tools menus; desktop MVP lives in `services/desktop`.
