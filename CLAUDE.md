...
- **Next (autonomous increments):** keep raising **fidelity + editing UX** — smoother selection/move/resize, text-annotation editing, image stamps, opacity/font controls, more polish; certified PKCS#7 e-signing (UX-S2) and redaction true-removal (UX-S5) remain open. Collaboration/Share is **deprioritized** per the user; revisit later (collab notes from the Phase-3 investigation are in git history / the `services/collab` audit).
